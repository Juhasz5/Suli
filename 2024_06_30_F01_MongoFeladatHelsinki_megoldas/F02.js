//2) Az előbb létrehozzott kollekciójába helyezze el a txt államányban található adatokat, dokumentumokban rendezve, az állományban az összetartozó adatok soronként vannak elhelyezve, elválasztó karakterük pedig " "!
...
function ObjektumFeltolto(feltoltendoElem) {
    ...
    for (let i = 0; i < feltoltendoElem.length; i++) {
        ...
        objektum.helyezes = Number(daraboltSor[0]);
        objektum.csapatMeret = Number(daraboltSor[1]);
        objektum.sportAg = daraboltSor[2];
        objektum.versenySzam = daraboltSor[3];
        beolvasottAdatok.push(objektum);
    }
    return beolvasottAdatok;
}

const helsinkiAdatok = ObjektumFeltolto(eredmenyek);
...
async function TobbDocumentumBeillesztes() {
    try {
        ...
        const muveletek = await db.collection("Helsinki").insertMany(helsinkiAdatok);
        console.log("A dokumentumok beillesztése a collectionbe sikeres volt!", muveletek.insertedCount, " új elem beillesztve.");
        client.close();
    }
    catch (err) {
